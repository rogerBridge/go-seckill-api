...
func LoadLimit() error {
	...
	for _, v := range purchaseLimit {
		logger.Info(v.ProductId, v.LimitNum, v.StartPurchaseDatetime, v.EndPurchaseDatetime)
		//err = conn.Send("hmset", "limit:"+strconv.Itoa(v.ProductId), "limitNum", v.LimitNum, "startPurchaseTime", v.StartPurchaseDatetime, "endPurchaseTime", v.EndPurchaseDatetime)
		//if err != nil {
		//	log.Println(err)
		//	return err
...
func (u *User) orderGenerator(productID string, purchaseNum int) (string, error) {
	...
	if err != nil {
		logger.Warnf("用户 %v 生成订单过程中出现了错误: %+v\n", u, err)
		return "", err
	}
	if ok == "OK" {
		logger.Warnf("%v 购买 %s %d件成功", u, productID, purchaseNum)
	}
	...
	if err != nil {
		logger.Warnf("%v productID convert from string to int　%v", u, err)
		return "", err
	}
	...
	if err != nil {
		logger.Warnf("%v json marshal error message: %v", u, err)
		return "", err
	}
...
func (u *User) Bought(productID string, purchaseNum int) error {
	...
	if err != nil {
		logger.Warnf("add bought info to %v:bought error message: %v", u, err)
		return err
	}
	...
	if flag == 0 {
		...
		if err != nil {
			logger.Warnf("when %v buy it, already exist, error message: %v", u, err)
			return err
		}
	}
	return nil
...
func (u *User) CancelBuy(orderNum string) error {
	...
	if err != nil {
		logger.Warnf("%+v 查询user:%s:order:%s 时出错!", u, u.userID, orderNum)
		return err
	}
	if isOrderExist == 0 {
		logger.Warnf("%+v 查询user:%s:order:%s 时不存在!", u, u.userID, orderNum)
		return fmt.Errorf("%v: 系统中没有找到该订单: %v", u, orderNum)
	}
	status, err := redis.String(conn1.Do("hget", "user:"+u.userID+":order:"+orderNum, "status"))
	if err != nil {
		logger.Warnf("%v 获取用户:%s订单:%s合法性的时候出现了错误", u, u.userID, orderNum)
		return err
	}
	if status != "process" {
		logger.Warnf("%+v 订单%s状态不对", u, orderNum)
	...
	if err != nil {
		logger.Warnf("hget user:%v:order:%v productID error", u, orderNum)
		return err
	}
	purchaseNum, err := redis.Int(conn1.Do("hget", "user:"+u.userID+":order:"+orderNum, "purchaseNum"))
	if err != nil {
		logger.Warnf("hget user:%s:order:%s purchaseNum error", u.userID, orderNum)
		return err
	}

	isExist, err := redis.Int(conn1.Do("hexists", "user:"+u.userID+":bought", productID))
	if err != nil {
		logger.Warnf("%v 查询user:%v:bought时出错!", u, u.userID)
		return err
	}
	if isExist == 0 {
		logger.Warnf("user:%v 没有购买过的东东:%v, 不可以取消哦~", u, productID)
	...
	if err != nil {
		logger.Warnf("%+v 获取已购买商品%s时出现错误! %+v", u, productID, err)
		return err
	}
	// 如果顾客购买了1件, 却要取消两件, 那就拒绝
	if !(existPurchaseNum >= purchaseNum) {
		logger.Warnf("%v 取消购买的数量不能大于购买的数量", u)
	...
	if err != nil {
		logger.Warnf("%+v 尝试更改订单: %s 状态时出现错误, 错误原因是: %v\n", u, orderNum, err) // 这里应该将订单状态还原, 并且将错误日志记录在案
	...
	err = conn.Send("hincrby", "store:"+productID, "storeNum", incrString)
	if err != nil {
		logger.Warnf("%v 返还库存 %v 时出错 %v", u, productID, err)
	...
	if err != nil {
		logger.Warnf("%+v 变更bought表: 'user:%s:bought'时出错%v", u, u.userID, err)
	...
	if err := orders.UpdateOrders("cancel", orderNum); err != nil {
		logger.Warnf("mysql处理更新orders表的时候出错 %v", err)
		return err
	}
